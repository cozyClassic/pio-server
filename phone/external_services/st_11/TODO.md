...
1. `월요금이 가장 비싼 요금제 옵션`은 DB에 없고, 오픈마켓에만 있음. (미끼 상품)
2. DB에 들어있는 상품은 마진이 6만원~8만원 들어가 있는 상태임.
3. `월요금이 비싼 요금제 옵션`은 마진이 1만원 수준이어야 함.
4. 따라서, 오픈마켓에 있는 `월요금이 비싼 요금제 옵션`은 DB에 저장된 product_option의 final_price 중 최저가인 것 보다 오히려 저렴해야 함 (수수료를 제외하면)
5. 그리고, DB에 있는 product_option이 얼마만큼의 마진을 남기고 있는지도 모르기 때문에 이를 입력받을 수 있는 칸이 필요함.

주어진상황 2 (옵션 가격 설정)

1. 요금제가 저렴해질수록 DB에 잇는 final_price가 더 비싸져야 하고, 결국 오픈마켓에 저장된 옵션 가격도 점점 올라야 함
2. 현재는 옵션 가격이 더 비싸지지 않고 오히려 저렴해지는 에러?가 있음
